    ...
    public String apply(String username, ModelMap model) {
        ...
        if (StringUtils.isNotBlank(username)) {
            ...
            if (user != null) {
                ...
                context.put("site", env.getProperty("site.website"));
                mailHelper.sendEmail(Consts.EMAIL_TEMPLATE_FORGOT, user.getEmail(), "找回密码-from一泓清水", context);
                data = Data.success("邮件发送成功", Data.NOOP);

                model.put("data", data);
                return view(Views.REGISTER_RESULT);
            } else {
                data = Data.failure("没有查到该用户，请检查用户名或者邮箱输入是否正确。");
            }
        }
        model.put("data", data);
